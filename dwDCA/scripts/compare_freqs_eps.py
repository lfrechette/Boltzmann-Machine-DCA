...
mycode = np.loadtxt('data/freq_diff_msa_ga.fas_none_1000_0.100000_1000_0.010000.txt')
rang= np.loadtxt('data/freq_diff_rang_lambda=0.01.txt')
rang_01= np.loadtxt('data/freq_diff_rang_eps=0.1.txt')


fig,ax=plt.subplots(1,3,figsize=(7,2.5))

titles=[r'my code, $\epsilon=0.1$', r'Ranganathan', r'Rang., $\epsilon=0.1$']
mylist = [mycode, rang, rang_01]
...
for j in range(3):
    ...
    ax[j].set_xlim([0,100])
plt.savefig('freq_diff_h_eps.png', dpi=300, bbox_inches='tight')
...
for j in range(3):
    ...
    ax[j].set_title(titles[j])
    ax[j].set_xlim([0,100])
plt.savefig('freq_diff_J_eps.png', dpi=300, bbox_inches='tight')
